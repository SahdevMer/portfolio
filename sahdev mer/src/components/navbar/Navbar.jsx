import React, { useState, useEffect } from "react";
import "flowbite"; // Ensure Flowbite CSS is imported if needed
import { Link } from "react-router-dom";

const Navbar = () => {
  const [navOpen, setNavOpen] = useState(false);
  const [dropdownOpen, setDropdownOpen] = useState(false);

  useEffect(() => {
    if (
      typeof window.Flowbite !== "undefined" &&
      typeof window.Flowbite.initDropdowns === "function"
    ) {
      window.Flowbite.initDropdowns();
    } else if (typeof window.initFlowbite === "function") {
      window.initFlowbite();
    } else {
    }

    return () => {};
  }, []);
  const toggleNav = () => {
    setNavOpen(!navOpen);
  };

  const toggleDropdown = () => {
    setDropdownOpen(!dropdownOpen);
  };

  return (
    <div>
      <nav className="border-gray-200 bg-black dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <a href="" className=" ">
            <h1 className="self-center text-2xl font-bold whitespace-nowrap text-white dark:text-white">
              Sahdev Mer
            </h1>
          </a>
          <div className="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            <button
              type="button"
              className="flex text-sm  bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600"
              id="user-menu-button"
              aria-expanded={dropdownOpen}
              data-dropdown-toggle="user-dropdown"
              data-dropdown-placement="bottom"
              onClick={toggleDropdown}
            >
              {/* User Icon can be replaced with an actual icon component */}
              <div className="relative">
                <img
                  className="w-10 h-10 rounded-full"
                  src="data:image/jpeg;base64,"
                  alt=""
                />
                <span class="bottom-0 left-7 absolute animate-ping w-2.5 h-2.5 bg-green-400 border-2 border-white dark:border-gray-800 rounded-full duration-300"></span>
              </div>
            </button>
            <div
              className={`z-50 justify-center flex ${
                dropdownOpen ? "" : "hidden"
              } text-base list-none  bg-gray-600 rounded-lg shadow-sm dark:bg-gray-700 dark:divide-gray-600`}
              id="user-dropdown"
            >
              <div className="w-30 relative h-30 flex items-center justify-center">
                <img
                  className="rounded-full p-5"
                  src="data:image/jpeg;base64,"
                  alt="Sahdev Mer"
                />
              </div>
              <div className="px-4 py-3">
                <span className="block px-4 py-2 text-sm text-gray-200 dark:text-gray-400">
                  <p className="text-xl text-white text-center">Sahdev Mer</p>
                </span>
                <strong className="block px-4 py-2 text-sm text-gray-200 bg-green-200 rounded-lg dark:text-gray-400">
                  <p className="text-sm text-green-400 dark:text-gray-500">
                    ACTIVE - Freelancer
                  </p>
                </strong>
                <span>
                  <p className="text-md px-4 py-1  text-gray-200 dark:text-gray-400">
                    Software Developer
                  </p>
                </span>
              </div>
            </div>
            <button
              data-collapse-toggle="navbar-user"
              type="button"
              className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
              aria-controls="navbar-user"
              aria-expanded={navOpen} // Use React state for aria-expanded
              onClick={toggleNav} // Toggle the mobile nav state
            >
              <span className="sr-only">Open main menu</span>
              <svg
                className="w-5 h-5"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 17 14"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M1 1h15M1 7h15M1 13h15"
                />
              </svg>
            </button>
          </div>
          <div
            className={`items-center justify-between ${
              navOpen ? "" : "hidden"
            } w-full md:flex md:w-auto md:order-1`}
            id="navbar-user"
          >
            <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg lg:bg-black-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0  dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 text-white">
              <li
                className="block py-2 px-3 text-white bg-blue-700 rounded-sm md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500 "
                aria-current="page"
              >
                <Link to="/" className="block py-2 px-3 text-white ">
                  Home
                </Link>
              </li>

              <li className="block py-2 px-3  rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 lg:text-white text-white">
                <Link
                  to="/about"
                  className="block py-2 px-3 hover:text-blue-500"
                >
                  About
                </Link>
              </li>

              <li className="block py-2 px-3  rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 lg:text-white text-white">
                <Link
                  to="/experience"
                  className="block py-2 px-3 hover:text-blue-500"
                >
                  Experience
                </Link>
              </li>

              <li className="block py-2 px-3  rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 lg:text-white text-white">
                <Link
                  to="/project"
                  className="block py-2 px-3 hover:text-blue-500"
                >
                  Projects
                </Link>
              </li>

              <li className="block py-2 px-3  rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 lg:text-white text-white">
                <Link
                  to="/contact"
                  className="block py-2 px-3 hover:text-blue-500"
                >
                  Contact
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </div>
  );
};

export default Navbar;
